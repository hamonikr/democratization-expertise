...
public class UsersService {
	...
	@Autowired
	UsersListRepository userListRepository;
...
//	}
	...
	public Page<Users> findAll(Pageable pageable) throws Exception {
		...
		System.out.println("sort=0==" + sort.length);
		System.out.println("sort=1==" + sort[0]);
//		System.out.println("sort===" + sort[1]);
	...
	// 목록
	public Page<Userslist> findAllaaa(Pageable pageable, String sortvote, String sortscore) throws Exception {
		int page = (pageable.getPageNumber() == 0) ? 0 : (pageable.getPageNumber() - 1);

		// 기본 정렬 설정
		String[] sort = pageable.getSort().toString().split(": ");
		String sortColumn = "totlikes";
		Direction direction = Sort.Direction.DESC;

		if (sortvote != null) {
			if (sortvote.contains("ASC")) {
		...
		if (sortscore != null) {
			if (sortscore.contains("ASC")) {
				direction = Sort.Direction.DESC;
				sortColumn = "userscore";
			} else {
				direction = Sort.Direction.ASC;
				sortColumn = "userscore";
			}
		...
		pageable = PageRequest.of(page, 30, new Sort(direction, sortColumn));
//		f(0, 5, Sort.by("price").descending().and(Sort.by("name")));
		System.out.println("===pageable.getSort()===" + pageable.getSort());

		Page<Userslist> list = userListRepository.findAllWithTeacher(pageable);

		System.out.println("=======================================================" + list.getSort());
		return list;
	}
