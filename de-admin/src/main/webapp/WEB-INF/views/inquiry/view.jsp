					...
					<div class="card-footer cont_btn_div">
						...
						<c:if test="${commentList == null}">
							<c:if test="${result.proc_stat ne '3'}">
								...
								<button type="button" class="btn btn-primary purple" id="request" onClick="requestToHead()">인베슘으로 요청</button>
							</c:if>
							<c:if test="${result.proc_stat eq '3'}">
								...
								<button type="button" class="btn btn-primary red" id="canel_request" onClick="cancelRequestToHead()">인베슘으로 요청 취소</button>
							</c:if>
						</c:if>
					</div>
				</div>
						...
						<div class="card card-primary">
							...
							<div class="card-body card-primary card-outline row">
								...
								<div class="col-lg-6">
									<div class="form-group">
										<label for="inputEmail3" class="col-form-label">담당자 배정</label>
											<span class="form-control form-control" name="assignee_up" id="assignee_up" >${commentList.assignee}</span>
									</div>
								</div>
								
								<div class="form-group col-lg-6">
									<label for="inputEmail3" class="col-form-label">처리 상태</label>
										<span class="form-control form-control" name="proc_stat" id="proc_stat" >
											<c:if test="${commentList.proc_stat eq 1 }"> 처리중</c:if>
											<c:if test="${commentList.proc_stat eq 2 }"> 완료</c:if>
										</span>
								</div>
								<div class="col-lg-6">
									<div class="form-group">
						...
						<div class="card card-primary" id="click_update" style="display: none">
							...
							<div class="card-body card-primary card-outline row">
								...
								<div class="col-lg-4">
									<div class="form-group">
										<label for="inputEmail3" class="col-form-label">담당자 배정</label>
										<select class="form-control" name="assignee_up" id="assignee_up" style="width: 100%;">
											<c:forEach var="item" items="${alist}" varStatus="status">
												<option value="${item.user_nm}" ${item.user_nm == commentList.assignee ? 'selected="selected"' : '' }> ${item.user_nm}</option>
											</c:forEach>
										</select>
									</div>
								</div>
...
function save(){
	document.frm.assignee.value = $("select[name=assignee]").val();
	//alert(assignee);
	...
	$.ajax({
		data : {title, comments, proc_stat, assignee},
...
function updateComment(){
	tmp = 1;
	 ...
	 var assignee = $("select[name=assignee_up]").val();
